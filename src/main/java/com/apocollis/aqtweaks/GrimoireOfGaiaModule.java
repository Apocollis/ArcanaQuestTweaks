...
public class GrimoireOfGaiaModule {
    ...
    public void onProjectileImpact(ProjectileImpactEvent event) {
        ...
        if (projectile != null && projectile.getClass().getName().startsWith("gaia.")) {
            ...
            if (projectile instanceof net.minecraft.entity.projectile.EntityFireball) {
                shooter = ((net.minecraft.entity.projectile.EntityFireball) projectile).shootingEntity;
            }
            long tick = player.world.getTotalWorldTime();
            gaiaDamageTracker.put(player.getUniqueID(), new GaiaDamageInfo(tick, shooter, projectile));
        }
    }
    ...
    public void onLivingHurt(LivingHurtEvent event) {
        ...
        if (source.getTrueSource() != null && source.getTrueSource().getClass().getName().startsWith("gaia.")) {
            gaiaDamageTracker.put(player.getUniqueID(), new GaiaDamageInfo(tick, source.getTrueSource(), null));
            return;
        }
        ...
        if (source.isUnblockable() || source.isDamageAbsolute() || source.isMagicDamage()) {
            GaiaDamageInfo entry = gaiaDamageTracker.get(player.getUniqueID());
            if (entry != null && entry.tick == tick) {
                ...
                player.attackEntityFrom(customSource, event.getAmount());
