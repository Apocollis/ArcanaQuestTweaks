...
public class StaminaModule {
    ...
    public void onEntityJoinWorldLowest(EntityJoinWorldEvent event) {
        ...
        if (!event.getWorld().isRemote && event.getEntity() != null) {
            ...
            if (entity instanceof net.minecraft.entity.projectile.EntityArrow) {
                net.minecraft.entity.Entity shooter = ((net.minecraft.entity.projectile.EntityArrow) entity).shootingEntity;
            ...
            if (throwerMP == null && entity instanceof net.minecraft.entity.projectile.EntityThrowable) {
                net.minecraft.entity.EntityLivingBase shooter = ((net.minecraft.entity.projectile.EntityThrowable) entity).getThrower();
                if (shooter instanceof EntityPlayerMP) {
                    throwerMP = (EntityPlayerMP) shooter;
                }
            }
            if (throwerMP == null && entity instanceof net.minecraftforge.fml.common.registry.IThrowableEntity) {
                net.minecraft.entity.Entity shooter = ((net.minecraftforge.fml.common.registry.IThrowableEntity) entity).getThrower();
                if (shooter instanceof EntityPlayerMP) {
                    throwerMP = (EntityPlayerMP) shooter;
                }
            }

            if (throwerMP != null && !Reflect.isCreative(throwerMP) && !throwerMP.isSpectator()) {
    ...
    private void handleServerBowDrawing(EntityPlayerMP player) {
        ...
        boolean isDrawing = player.isHandActive() && player.getActiveItemStack().getItem() instanceof ItemBow
                            && !isThrowingWeapon(player.getActiveItemStack());
    ...
    /**
     * Handles stamina drain for throwing weapons (Spartan Weaponry javelins,
     * throwing knives, throwing axes, daggers).
     *
     * Holding drawn: half the bow drain rate (bowHoldInterval * throwingHoldIntervalMultiplier).
     * Initial draw: bowDrawCost / throwingDrawCostDivisor.
     * Releasing the throw: flat throwingReleaseCost (same as light weapon attack).
     */
    private void handleServerThrowingHold(EntityPlayerMP player) {
        ...
        boolean isHoldingThrow = player.isHandActive() && isThrowingWeapon(player.getActiveItemStack());
        boolean wasHolding = player.getEntityData().getBoolean("StaminaTweaksThrowActive");

        if (isHoldingThrow) {
            if (!wasHolding) {
                // Free to aim/draw throwing weapon!
                player.getEntityData().setBoolean("StaminaTweaksThrowActive", true);
                player.getEntityData().setInteger("StaminaTweaksThrowTicks", 0);
            }
        } else {
            if (wasHolding) {
                // Reset state flag (stamina cost of 1 feather is handled in onItemUseStop upon release)
                player.getEntityData().setBoolean("StaminaTweaksThrowActive", false);
                player.getEntityData().setInteger("StaminaTweaksThrowTicks", 0);
            }
        }
    }
    ...
    public void onItemUseStop(LivingEntityUseItemEvent.Stop event) {
        ...
        if (isThrowingWeapon(stack)) {
            ...
            if (Reflect.hasEnoughStamina(playerMP, releaseCost)) {
                ...
                // Cancel the release event, preventing the weapon from being thrown
                event.setCanceled(true);
                playerMP.resetActiveHand();
            }
        }
