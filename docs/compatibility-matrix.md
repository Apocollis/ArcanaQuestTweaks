...
| `dynamicswordskills` | `1.12.2-DynamicSwordSkills-6.0.1.jar` (+ `SwordSkillsApi-1.1.0`) | after | optional `mixins.aqtweaks.dss.json` | **yes** (`EntitySwordBeam`, `SkillActive`, `DSSKeyHandler`, `PacketDispatcher`, `OpenGuiPacket`) | yes | **yes** | stamina, [reskillable.md](reskillable.md) | `MixinSkillActive` → `SkillActive.trigger`. `MixinEntitySwordBeam` stamps magic + unblockable on impact. `MixinDSSKeyHandler` (client) cancels `onKeyPressed` for key 0 and marks a real skills-key press. `ClientProxy` always registers the skills-GUI tick and `/dssgui` (imports DSS; pack ships the jar). Compiles Elenai for spend |
| `baubles` | `BaublesEX-1.12.2-2.3.7.jar` | after | optional `mixins.aqtweaks.baubles.json` | **yes** (`KeyBindings`, `PacketHandler`, `PacketOpen`, `ClientEventHandler`) | yes | **yes** | [minemenu.md](minemenu.md), [thaumcraft.md](thaumcraft.md) | Client tick sends `PacketOpen(EXPANSION)` when `isPressed()` and Baubles did not already send. `MixinClientEventHandler` marks `onKeyInput`'s `sendToServer`. Do not call `Keyboard.isKeyDown`. Focus pouch offset stays in the Thaumcraft mixins |
...
| `dynamicstealth` | `DynamicStealth-1.12.2.120.jar` | omitted | mixin lives in `mixins.aqtweaks.dss.json` | **yes** (`Sight`, `StealthAttackEvent`, `Attributes`) | yes | **yes** | [reskillable.md](reskillable.md) | Opportunistic redirects `canSee`. Awareness cancels `StealthAttackEvent`. Taunt and Low Profile edit threat generation. Dark Vision makes `hasNightvision` true |
| `playerrevive` | `PlayerRevive_v1.2.40_mc1.12.2.jar` | omitted | optional `mixins.aqtweaks.playerrevive.json` | **yes** (`Revival`) | yes | **yes** | [reskillable.md](reskillable.md) | Fast Revive adds helper progress in `tick` |
| `prospectus` | `prospectus-1.8.jar` | omitted | optional `mixins.aqtweaks.prospectus.json` | **yes** (`ItemProspector`, `Prospectus.isStackWhitelisted`) | skip json if absent | **yes** | [reskillable.md](reskillable.md) | Prospector replaces pick accuracy and outlines the blocks that click counted. Radius and chat text stay Prospectus |
| `farmersdelight` | `FarmersDelightLegacy-1.1.7.jar` | omitted | — | **yes** (`BlockTomatoVine`) | yes | **yes** | [reskillable.md](reskillable.md) | Orchard fruit. Tomato vines skip Bountiful and the farming drip |
| `extradelight` | `extra-delight-legacy-1.1.6.jar` | omitted | — | **yes** (`BlockOrchardLeaves`) | yes | **yes** | [reskillable.md](reskillable.md) | Orchard fruit on Extra Delight orchard leaves |
...
| `somnia` | `Somnia-1.0.1.jar` | omitted | optional `mixins.aqtweaks.somnia.json` | **yes** (`SomniaUtil`) | yes | **yes** | [somnia.md](somnia.md) | `MixinSomniaUtil` `@Overwrite` `chunkLightCheck`; `SomniaOptimizationHandler` proximity-priority drain (<4096 within 2 chunks drains 1t, else 20t) & fixes duplicate calls |
