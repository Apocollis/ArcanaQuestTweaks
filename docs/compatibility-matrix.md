...
| `dynamicswordskills` | `1.12.2-DynamicSwordSkills-6.0.1.jar` (+ `SwordSkillsApi-1.1.0`) | after | optional `mixins.aqtweaks.dss.json` | **yes** (`EntitySwordBeam`, `SkillActive`, `DSSKeyHandler`, `PacketDispatcher`, `OpenGuiPacket`) | yes | **yes** | stamina, [reskillable.md](reskillable.md) | `MixinSkillActive` → `SkillActive.trigger`. `MixinEntitySwordBeam` stamps magic + unblockable on impact. `ClientProxy` always registers the skills-GUI tick and `/dssgui` (imports DSS; pack ships the jar). Compiles Elenai for spend |
| `baubles` | `BaublesEX-1.12.2-2.3.7.jar` | after | — | **yes** (`KeyBindings`, `PacketHandler`, `PacketOpen`) | yes | **yes** | [minemenu.md](minemenu.md), [thaumcraft.md](thaumcraft.md) | Client tick sends `PacketOpen(EXPANSION)` when the Baubles key `isPressed()` and the hardware key is up. Focus pouch offset stays in the Thaumcraft mixins |
...
| MineMenu (no jar) | — | omitted | `MixinEntityRendererMouse` in **required** json client; `MixinMinecraftMouseGrab` in **early** json client | no | always (vanilla client) | — | [minemenu.md](minemenu.md) | Skip `turn` while a screen is open. Re-grab and drain DX/DY when `displayGuiScreen` returns to a null screen. Do not late-mixin `Minecraft` |
