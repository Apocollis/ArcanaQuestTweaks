...
| MineMenu | Reskillable: look frozen while open. Close from a corner: cursor grabbed at center, no yaw snap, next look starts from center. Inventory and Esc the same. MineMenu keybind for Baubles opens the expanded GUI once; the real Baubles key still once. DSS Skills GUI once; `/dssgui` opens it. Boot: no `Minecraft was loaded too early` / `EntityRenderer was loaded too early`. See [minemenu.md](minemenu.md), [stamina.md](stamina.md) |
...
| Reskillable | Attack 16 → +2 damage; Mining Expert wood pick drops diamond ore; stamina perks on tree; Magic school mutex + follow-ups. Full list: [reskillable.md](reskillable.md) |
